...
// task start
gulp.task('json', () => {
  return gulp.src([`${src}/**/*.json`, `!${src}/project.config.json`]).pipe(isProd ? jsonminify() : through.obj()).pipe(gulp.dest(dist))
})
...
gulp.task('clean', () => {
  return del(['dist/**', '!dist', '!dist/project.config.json'])
})

gulp.task('dev', ['clean'], () => {
  runSequence('json', 'images', 'wxml', 'wxss', 'js', 'wxs', 'cloud', 'watch', 'watch:cloud')
})
...
gulp.task('cloud', () => {
  return gulp
    ...
    .pipe(gulp.dest(`${cloudDist}/cloud-functions`))
})
